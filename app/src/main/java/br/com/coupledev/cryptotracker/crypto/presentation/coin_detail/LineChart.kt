...
fun LineChart(
    ...
    var xLabelWith by remember {
        mutableFloatStateOf(0f)
    }

    LaunchedEffect(key1 = xLabelWith) {
        onXLabelWidthChange(xLabelWith)
    }
    ...
    Canvas(
        modifier = modifier
            .fillMaxSize()
            .pointerInput(drawPoints, xLabelWith) {
                detectHorizontalDragGestures { change, _ ->
                    val newSelectedDataPointIndex = getSelectedDataPointIndex(
                        touchOffsetX = change.position.x,
                        triggerWidth = xLabelWith,
                        drawPoints = drawPoints
                    )
                    isShowingDataPoints =
                        (newSelectedDataPointIndex + visiblePointIndices.first) in
                                visiblePointIndices
        ...
        val labelCountExcludingLastLabel = ((labelViewPortHeightPx /
        ...
        xLabelWith = maxXLabelWidth + xAxisLabelSpacingPx
        xLabelTextLayoutResults.forEachIndexed { index, result ->
            val x = viewPortLeftX + xAxisLabelSpacingPx / 2f + xLabelWith * index
        ...
        drawPoints = visiblePointIndices.map {
            val x = viewPortLeftX + (it - visiblePointIndices.first) * xLabelWith + xLabelWith / 2f
